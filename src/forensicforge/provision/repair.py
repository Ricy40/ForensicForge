...
Week 6 found three distinct, reproducible generation bugs by actually
provisioning real specs (see docs/METHODOLOGY.md): a YAML nested-quote
failure, an `ansible.builtin.lineinfile` `line:` value using a regex
backreference (which that parameter doesn't support - it's written
literally), and an `ansible.builtin.user` task misusing `path:` (a
`file`-module parameter). None are one-off typos worth patching by hand
in a generated role; each is a *pattern* the LLM produces repeatably
given a similar prompt. This module recognizes each pattern generically
(by shape, not by matching the exact broken text seen this week) and
repairs it, rather than leaving every future occurrence to fail the same
way. Every repair is recorded in the string list each function returns,
never applied silently - see write_ansible_role()'s use of these.
